...
public class CloudSolrjClient extends SolrClient {

    protected static final Logger log = LoggerFactory.getLogger(CloudSolrjClient.class);
     ...
     *
     * @param zkHost The client endpoint of the zookeeper quorum containing the cloud
     ...
     */
    public CloudSolrjClient(String zkHost) {
        this.zkHost = zkHost;
     ...
     *                   <p>
     *                   "zoo1.example.com:2181,zoo2.example.com:2181,zoo3.example.com:2181"
     * @param httpClient the {@link HttpClient} instance to be used for all requests. The
     *                   provided httpClient should use a multi-threaded connection manager.
     */
    public CloudSolrjClient(String zkHost, HttpClient httpClient) {
        this.zkHost = zkHost;
     ...
     * {@link #CloudSolrjClient(String)} instead.
     *
     * @param zkHosts A Java Collection (List, Set, etc) of HOST:PORT strings, one for
     *                each host in the zookeeper ensemble. Note that with certain
     *                Collection types like HashSet, the order of hosts in the final
     *                connect string may not be in the same order you added them.
     * @param chroot  A chroot value for zookeeper, starting with a forward slash. If no
     *                chroot is required, use null.
     * @throws IllegalArgumentException if the chroot value does not start with a forward slash.
     * @see #CloudSolrjClient(String)
     */
    public CloudSolrjClient(Collection<String> zkHosts, String chroot) {
     ...
     * @param chroot     A chroot value for zookeeper, starting with a forward slash. If no
     *                   chroot is required, use null.
     * @param httpClient the {@link HttpClient} instance to be used for all requests. The provided httpClient should use a
     *                   multi-threaded connection manager.
     * @throws IllegalArgumentException if the chroot value does not start with a forward slash.
     * @see #CloudSolrjClient(String)
     */
    public CloudSolrjClient(Collection<String> zkHosts, String chroot, HttpClient httpClient) {
     ...
     * @see #CloudSolrjClient(String) for full description and details on zkHost
     */
    public CloudSolrjClient(String zkHost, boolean updatesToLeaders) {
     ...
     * @see #CloudSolrjClient(String) for full description and details on zkHost
     */
    public CloudSolrjClient(String zkHost, boolean updatesToLeaders, HttpClient httpClient) {
        this.zkHost = zkHost;
        this.clientIsInternal = httpClient == null;
        this.myClient = httpClient == null ? HttpClientUtil.createClient(null) : httpClient;
        this.lbClient = new LBHttpSolrClient(myClient);
    ...
    /**
     * @param zkHost   A zookeeper client endpoint.
     * @param lbClient LBHttpSolrServer instance for requests.
     * @see #CloudSolrjClient(String) for full description and details on zkHost
     */
    public CloudSolrjClient(String zkHost, LBHttpSolrClient lbClient) {
     ...
     * @see #CloudSolrjClient(String) for full description and details on zkHost
     */
    public CloudSolrjClient(String zkHost, LBHttpSolrClient lbClient, boolean updatesToLeaders) {
        this.zkHost = zkHost;
    ...
    private NamedList<Object> directUpdate(AbstractUpdateRequest request, String collection, ClusterState clusterState) throws SolrServerException {
        ...
        if (parallelUpdates) {
            ...
            for (final Map.Entry<String, LBHttpSolrClient.Req> entry : routes.entrySet()) {
                ...
                try {
                    MDC.put("CloudSolrjClient.url", url);
                    ...
                    MDC.remove("CloudSolrjClient.url");
                }
            }
    ...
    protected NamedList<Object> sendRequest(SolrRequest request, String collection)
        ...
        LBHttpSolrClient.Rsp rsp = lbClient.request(req);
        rsp.getResponse().add("server", rsp.getServer());
