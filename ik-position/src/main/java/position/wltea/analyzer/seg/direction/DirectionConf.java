package position.wltea.analyzer.seg.direction;

import com.jorado.ik.Logger;
import position.wltea.analyzer.seg.conf.LocalSettings;
...
public class DirectionConf {
    ...
    private List<String> getdicListByFile(String filePath) {
        ...
        try {
            ...
            Logger.error("读取词库位置异常IOException", e);
            e.printStackTrace();
        }
        String ss;
        try {
            ...
            Logger.error("读文件内容异常IOException", e);
            e.printStackTrace();
        } finally {
            try {
                if (null != reader) {
                ...
                Logger.error("文件流关闭异常IOException", e);
                e.printStackTrace();
