package com.jorado.ik.admin.creator;

import com.jorado.ik.admin.model.TermFrequency;
import com.jorado.ik.admin.util.CommonUtils;
import com.jorado.ik.admin.util.FileUtils;
...
public class CompanyWordCreator {
    ...
    private Set<String> getNewWord(String data, Map<String, Integer> highFreWordMap) {
        ...
        for (int i = 0; i < lexemeList.size(); i++) {
            ...
            if (lexeme.getLexemeText().length() == 1) {
                ...
                if (!StringUtils.isNullOrWhiteSpace(newWord)) {
                    results.add(newWord);
                }
                if (newWord.length() > 3) {
    ...
    private Set<String> fmmSegWord(String word, Map<String, Integer> term2frequency, int minFre) {
        ...
        // 记录未分词成功的词
        String errorWord = "";
        while (!StringUtils.isNullOrWhiteSpace(word)) {
            len = word.length();
    ...
    private boolean isRegion(String word) {
        ...
        if (StringUtils.isNullOrWhiteSpace(word) || word.length() < 3) {
            return isRegion;
        }
